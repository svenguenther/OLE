...
class Sampler(BaseClass):
    ...
    def compute_loglike_from_parameters_differentiable(self, parameters):
        ...
        if not self.emulator.trained:
            ...
            print("Emulator sample loglikes: ", emulator_sample_loglikes)
...
class NUTSSampler(Sampler):
    ...
    def run_mcmc(self, nsteps, **kwargs):
        ...
        # puntjes
        thetas = jnp.zeros((self.M_adapt + nsteps + 1, self.ndim))
        logp = jnp.zeros(self.M_adapt + nsteps + 1)

        # number of rounds for the NUTS sampler to run and check the performance of the emulator
        nrounds = int(jnp.ceil((self.M_adapt + nsteps + 1)/self.NUTS_batch_size))
        ...
        for i in range(nrounds):
            for j in range(self.NUTS_batch_size):
                ...
                while s == 1:
                    ...
                    key, thetas[i*self.NUTS_batch_size+j+1], n, s, j = self._trajectory_iteration_update(thetas[i*self.NUTS_batch_size+j+1], n, s, j, theta_m, r_m, theta_p, r_p, theta_f, n_f, s_f, key)

                print(thetas[i*self.NUTS_batch_size+j+1])
