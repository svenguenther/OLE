...
sampling_settings_profile = {
    ...
    'n_restarts': 3,
    'profiled_parameter': {'name': 'h', 'range': [0.6, 0.8], 'n_samples': 10},
}


# load sampler 
from OLE.sampler import EnsembleSampler, Sampler, NUTSSampler, MinimizeSampler, ProfileLikelihoodSampler
my_sampler_NUTS = NUTSSampler()
my_sampler_profile = ProfileLikelihoodSampler()
...
# now minimize
my_sampler_profile.initialize(theory=my_theory, 
                      likelihood_collection=my_likelihood_collection, 
                      parameters=my_parameters, 
                      emulator_settings = emulator_settings,
                      likelihood_collection_settings = my_likelihood_collection_settings,
                      theory_settings = theory_settings,
                      sampling_settings = sampling_settings_profile,
                    #   emulator = my_sampler_NUTS.emulator,
                    #   debug = True
                      )
my_sampler_profile.minimize()
